...
class ChimiotherapieViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields   = ['patient', 'statut', 'intention', 'ligne', 'diagnostic']
    search_fields      = ['protocole', 'patient__nom', 'patient__registration_number']
    ordering           = ['-date_debut']

    def get_queryset(self):
        qs = Chimiotherapie.objects.select_related(
            'patient', 'diagnostic',
            'diagnostic__topographie', 'diagnostic__morphologie',
        ).prefetch_related('medicaments')
        pid = self.request.query_params.get('patient_id')
        if pid:
            qs = qs.filter(patient_id=pid)
        did = self.request.query_params.get('diagnostic_id')
        if did:
            qs = qs.filter(diagnostic_id=did)
        return qs
...
class RadiotherapieViewSet(viewsets.ModelViewSet):
    ...
    filter_backends    = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields   = ['patient', 'statut', 'technique', 'intention', 'diagnostic']
    search_fields      = ['site_irradie', 'patient__nom', 'patient__registration_number']
    ordering           = ['-date_debut']

    def get_queryset(self):
        qs = Radiotherapie.objects.select_related(
            'patient', 'diagnostic',
            'diagnostic__topographie', 'diagnostic__morphologie',
        )
        pid = self.request.query_params.get('patient_id')
...
class ChirurgieViewSet(viewsets.ModelViewSet):
    ...
    filter_backends    = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields   = ['patient', 'statut', 'type_chirurgie', 'marges_resection', 'diagnostic']
    search_fields      = ['intitule_acte', 'patient__nom', 'patient__registration_number']
    ordering           = ['-date_debut']

    def get_queryset(self):
        qs = Chirurgie.objects.select_related(
            'patient', 'diagnostic',
            'diagnostic__topographie', 'diagnostic__morphologie',
        )
        pid = self.request.query_params.get('patient_id')
...
class HormonotherapieViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields   = ['patient', 'statut', 'type_hormonotherapie', 'diagnostic']
    search_fields      = ['molecule', 'patient__nom']
    ordering           = ['-date_debut']

    def get_queryset(self):
        qs = Hormonotherapie.objects.select_related(
            'patient', 'diagnostic',
            'diagnostic__topographie', 'diagnostic__morphologie',
        )
        pid = self.request.query_params.get('patient_id')
...
class ImmunotherapieViewSet(viewsets.ModelViewSet):
    ...
    filter_backends    = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields   = ['patient', 'statut', 'type_immunotherapie', 'diagnostic']
    search_fields      = ['molecule', 'biomarqueur_cible', 'patient__nom']
    ordering           = ['-date_debut']

    def get_queryset(self):
        qs = Immunotherapie.objects.select_related(
            'patient', 'diagnostic',
            'diagnostic__topographie', 'diagnostic__morphologie',
        )
        pid = self.request.query_params.get('patient_id')
...
class TraitementsViewSet(viewsets.ViewSet):
    """Vue consolidée : tous les traitements d'un patient ou d'un diagnostic."""
    permission_classes = [IsAuthenticated]
    ...
    def par_patient(self, request):
        ...
        return Response({
            'chimiotherapies': ChimiotherapieListSerializer(
                Chimiotherapie.objects.filter(patient_id=pid)
                    .select_related('diagnostic')
                    .prefetch_related('medicaments'),
                many=True,
            ).data,
            'radiotherapies': RadiotherapieSerializer(
                Radiotherapie.objects.filter(patient_id=pid).select_related('diagnostic'),
                many=True,
            ).data,
            'chirurgies': ChirurgieSerializer(
                Chirurgie.objects.filter(patient_id=pid).select_related('diagnostic'),
                many=True,
            ).data,
            'hormonotherapies': HormonotherapieSerializer(
                Hormonotherapie.objects.filter(patient_id=pid).select_related('diagnostic'),
                many=True,
            ).data,
            'immunotherapies': ImmunotherapieSerializer(
                Immunotherapie.objects.filter(patient_id=pid).select_related('diagnostic'),
                many=True,
            ).data,
        })

    @action(detail=False, methods=['get'])
    def par_diagnostic(self, request):
        """Retourne tous les traitements liés à un diagnostic précis."""
        did = request.query_params.get('diagnostic_id')
        if not did:
            return Response({'error': 'diagnostic_id requis'}, status=400)
        return Response({
            'chimiotherapies': ChimiotherapieListSerializer(
                Chimiotherapie.objects.filter(diagnostic_id=did).prefetch_related('medicaments'),
                many=True,
            ).data,
            'radiotherapies': RadiotherapieSerializer(
                Radiotherapie.objects.filter(diagnostic_id=did),
                many=True,
            ).data,
            'chirurgies': ChirurgieSerializer(
                Chirurgie.objects.filter(diagnostic_id=did),
                many=True,
            ).data,
            'hormonotherapies': HormonotherapieSerializer(
                Hormonotherapie.objects.filter(diagnostic_id=did),
                many=True,
            ).data,
            'immunotherapies': ImmunotherapieSerializer(
                Immunotherapie.objects.filter(diagnostic_id=did),
                many=True,
            ).data,
        })
