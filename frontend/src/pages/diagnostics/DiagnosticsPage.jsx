...
const TRAITEMENT_CFG = {
  traite:     { label:'Traité',     color:'#00e5a0', bg:'rgba(0,229,160,0.10)',  border:'rgba(0,229,160,0.25)'  },
  en_cours:   { label:'En cours',   color:'#f5a623', bg:'rgba(245,166,35,0.10)', border:'rgba(245,166,35,0.25)' },
  non_traite: { label:'Non traité', color:'#ff4d6a', bg:'rgba(255,77,106,0.10)', border:'rgba(255,77,106,0.25)' },
};

function StageBadge({ stade, label }) {
  const c = STADE_COLORS[stade] || STADE_COLORS['U'];
  return <span style={{ padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:700, background:c.bg, color:c.color, border:`1px solid ${c.border}`, fontFamily:'var(--font-mono)' }}>{label||stade}</span>;
}
function TNMBadge({ tnm }) {
  if (!tnm||tnm==='—') return <span style={{ color:'var(--text-muted)', fontSize:12 }}>—</span>;
  return <span style={{ padding:'2px 8px', borderRadius:6, fontSize:11, background:'rgba(0,168,255,0.08)', border:'1px solid rgba(0,168,255,0.2)', color:'#00a8ff', fontFamily:'var(--font-mono)', fontWeight:600 }}>{tnm}</span>;
}
...
export default function DiagnosticsPage() {
  ...
  return (
    <AppLayout title="Diagnostics – ICD-O-3 · TNM">
      ...
      {/* Légende statut traitement */}
      <div style={{ display:'flex', gap:16, marginBottom:10, paddingLeft:4, flexWrap:'wrap', alignItems:'center' }}>
        <span style={{ fontSize:10, color:'var(--text-muted)' }}>Cliquer sur le nom du patient pour ouvrir la fiche</span>
        {Object.entries(TRAITEMENT_CFG).map(([k,v])=>(
      ...
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-light)', borderRadius:'var(--radius-md)', overflow:'hidden' }}>
        {loading ? <Loader/> : diagnostics.length===0 ? <Empty/> : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {diagnostics.map((d, i) => {
                const traitCfg = TRAITEMENT_CFG[d.statut_traitement] || null;
                    ...
                    <td style={tdStyle}>
                      ...
                      >
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          {traitCfg && (
                            <span title={traitCfg.label} style={{ width:7, height:7, borderRadius:'50%', background:traitCfg.color, flexShrink:0, display:'inline-block',
                              animation: d.statut_traitement==='en_cours'?'pulse-dot 1.4s ease-in-out infinite':'none' }}/>
                          )}
                        ...
                        <span style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text-muted)', paddingLeft:13 }}>{d.patient_numero}</span>
                      </div>
                    </td>
...
function ChartCard({ title, data=[], labelKey, valueKey, color }) {
  ...
  return (
    <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-light)', borderRadius:'var(--radius-md)', padding:'16px 18px' }}>
      <div style={{ fontSize:12, fontWeight:600, color:'var(--text-secondary)', marginBottom:14, textTransform:'uppercase', letterSpacing:.5 }}>{title}</div>
      {(data||[]).slice(0,6).map((item,i)=>(
        <div key={i} style={{ marginBottom:8 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
            <span style={{ fontSize:11, color:'var(--text-secondary)', maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item[labelKey]||'—'}</span>
...
function PageBtn({ children, disabled, onClick }) {
  return <button onClick={onClick} disabled={disabled} style={{ padding:'6px 14px', background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:6, color:'var(--text-secondary)', fontSize:12, cursor:disabled?'not-allowed':'pointer', opacity:disabled?0.4:1 }}>{children}</button>;
}
function Loader() {
...
const thStyle    = { padding:'10px 12px', textAlign:'left', fontSize:10, fontWeight:600, letterSpacing:.5, color:'var(--text-muted)', textTransform:'uppercase', borderBottom:'1px solid var(--border)', whiteSpace:'nowrap' };
