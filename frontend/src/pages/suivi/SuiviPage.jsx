...
const EVENEMENT_COLORS = {
  hospit:   '#00a8ff', complic: '#f5a623', recidive: '#ff4d6a',
  meta:     '#ff7832', toxicite:'#9b8afb', deces:    '#6b7280',
  remission:'#00e5a0', autre:   '#9ca3af',
};
...
function PSBadge({ ps }) {
  ...
  return (
    <span style={{ padding:'2px 8px', borderRadius:6, fontSize:11, fontWeight:700,
      fontFamily:'var(--font-mono)', background:`${colors[ps]}18`, color:colors[ps], border:`1px solid ${colors[ps]}30` }}>
      PS {ps}
    </span>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <button onClick={onNew} style={addBtnStyle('#9b8afb')}>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr style={{ background:'var(--bg-elevated)' }}>
              {['Patient','Type','Date','PS ECOG','Poids','Évolution','Statut','Prochaine RDV',''].map(h => (
...
// ── Section événements ────────────────────────────────────────────
function SectionEvenements({ onNew }) {
  const navigate = useNavigate();
  const [data, setData]   = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      const params = {};
      if (typeFilter) params.type_evenement = typeFilter;
      const { data: res } = await suiviService.evenements.list(params);
      setData(res.results || res);
    ...
    <div>
      <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
        <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} style={selSt}>
          ...
          <option value="toxicite">Toxicité</option>
          <option value="deces">Décès</option>
          <option value="remission">Rémission</option>
          <option value="autre">Autre</option>
        </select>
        <button onClick={onNew} style={addBtnStyle('#ff7832')}>
          + Nouvel événement
        </button>
      </div>

      <TableCard loading={loading} empty={data.length === 0}  emptyText="Aucun événement enregistré">
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr style={{ background:'var(--bg-elevated)' }}>
              {['Patient','Type','Date','Sévérité','Description','Résolu',''].map(h => (
                <th key={h} style={thSt}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.map((e, i) => {
              const typeColor = EVENEMENT_COLORS[e.type_evenement] || '#9ca3af';
              ...
              return (
                ...
                >
                  ...
                  <td style={tdSt}>
                    <Badge label={e.type_label} color={typeColor} />
                  </td>
                  <td style={{ ...tdSt, fontFamily:'var(--font-mono)', fontSize:12 }}>{new Date(e.date_evenement).toLocaleDateString('fr-DZ')}</td>
                  <td style={tdSt}>
                    {e.severite ? <span style={{ fontFamily:'var(--font-mono)', fontSize:12, fontWeight:700, color:sevColor }}>G{e.severite}</span> : <span style={{ color:'var(--text-muted)' }}>—</span>}
                  </td>
                  ...
                  <td style={tdSt}>
                    <span style={{ fontSize:11, color: e.resolu ? '#00e5a0' : '#f5a623' }}>
                      {e.resolu ? '✓ Résolu' : ' En cours'}
                    </span>
                  </td>
                  <td style={tdSt} onClick={ev => ev.stopPropagation()}>
                    <button style={viewBtnSt} onClick={() => navigate(`/suivi/evenements/${e.id}`)}>Voir</button>
                  </td>
                </tr>
              );
            })}
...
export default function SuiviPage() {
  ...
  const TABS = [
    ...
    { key:'evenements',    label:' Événements',    color:'#ff7832' },
    ...
    <AppLayout title="Suivi Clinique">
      ...
      {stats && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:24 }}>
          <StatCard label="Consultations totales" value={stats.total}           color="#9b8afb" />
          <StatCard label="À venir"               value={aVenir.length}         color="#00a8ff" />
          <StatCard label="Progressions"          value={(stats.par_evolution||[]).find(e=>e.evolution_maladie==='progression')?.n || 0} color="#ff4d6a" />
          <StatCard label="PS ECOG moyen"         value={stats.ps_ecog_moyen ? parseFloat(stats.ps_ecog_moyen).toFixed(1) : '—'} color="#f5a623" />
        </div>
      )}
      ...
      <div style={{ display:'flex', background:'var(--bg-card)', border:'1px solid var(--border-light)', borderRadius:'var(--radius-md)', overflow:'hidden', marginBottom:16 }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{ flex:1, padding:'12px', background:'none', border:'none', borderBottom:`2px solid ${tab===t.key?t.color:'transparent'}`, color:tab===t.key?t.color:'var(--text-muted)', fontSize:13, fontWeight:tab===t.key?600:400, cursor:'pointer', fontFamily:'var(--font-body)' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {/* Tab content */}
      {tab === 'consultations' && (
        <SectionConsultations onNew={() => navigate('/suivi/consultations/nouveau')} />
      )}
      {tab === 'evenements' && (
        <SectionEvenements onNew={() => navigate('/suivi/evenements/nouveau')} />
      )}
      {tab === 'agenda' && (
        <AgendaAVenir data={aVenir} />
      )}
    </AppLayout>
  );
}
...
function TableCard({ loading, empty, emptyIcon, emptyText, children }) {
  return (
